...
def train_cramer_dcgan(data, config):
    ...
    with training_graph.as_default():
        ...
        with tf.Session() as sess:
            ...
            load_checkpoint(sess, gan.saver, 'cramer_dcgan', checkpoint_dir, step=config.save_every_step)
            ...
            for epoch in range(epoch, epoch + config.epoch):
                ...
                for idx in range(0, num_batches):
                    ...
                    if config.save_every_step:
                        save_checkpoint(sess, gan.saver, 'cramer_dcgan', checkpoint_dir, global_step, step=True)

                    if config.verbose:
                        print("Epoch: [%2d] Step: [%4d/%4d] time: %4.4f, c_loss: %.8f, g_loss: %.8f" \
                                % (epoch, idx, num_batches, time.time() - start_time, gan.L_critic.eval(feed_dict={gan.images: batch_images}), gan.L_surrogate.eval(feed_dict={gan.images: batch_images})))

                    elif global_step%100 == 0:
                        print("Epoch: [%2d] Step: [%4d/%4d] time: %4.4f"%(epoch, idx, num_batches, time.time() - start_time))

                # save a checkpoint every epoch
                save_checkpoint(sess, gan.saver, 'cramer_dcgan', checkpoint_dir, epoch)
                sess.run(gan.increment_epoch)
